...
public class HashDoubleHashingTable {

    @Test
    public void createTableForHash() {
        List<String> words = initializeWordsList();
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("doubleTable.txt"))) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                for(int j = 0; j < 30; j++){
                    ...
                    Method hashFunc = HashDoubleHashing.class.getDeclaredMethod("hashFunc", int.class, int.class);
                    hashFunc.setAccessible(true);
